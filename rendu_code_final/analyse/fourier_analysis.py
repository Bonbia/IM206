...
# ─── Chemins (relatifs à la racine du dépôt, peu importe d'où tu lances) ──────
REPO_ROOT  = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
IMAGE_PATH = os.path.join(REPO_ROOT, "img", "baboon.png")  # ← change ici si besoin
OUT_DIR    = os.path.join(REPO_ROOT, "nos_tests/results")
TARGET     = 350        # taille intermédiaire de rééchantillonnage (512→TARGET→512)
...
def formater(img, taille, filtre):
...
def profil_1d(img, direction="horizontal"):
    ...
    return np.fft.fftshift(np.abs(F).mean(axis=1 - axe)).astype(np.float32)


def pics_theoriques(taille_orig, taille_cible, max_k=4):
    delta = abs(taille_orig - taille_cible)
    return [k * delta for k in range(1, max_k + 1) if k * delta <= taille_orig // 2]
...
# ─── Figure 1 : analyse complète ──────────────────────────────────────────────

def figure_analyse(img_orig, taille_cible, chemin_sortie, direction="horizontal"):
    H, W      = img_orig.shape
    reimages  = {f: formater(img_orig, taille_cible, f) for f in FILTRES}
...
# ─── Figure 2 : 4 composantes spectrales ──────────────────────────────────────
...
def figure_4composantes(img_orig, taille_cible, chemin_sortie):
    ...
    for ligne, (nom, cmap) in enumerate(zip(noms_comp, cmaps)):
        def afficher(ax, img, titre, couleur, _ligne=ligne):
            x     = rank_transform(img)
            F     = np.fft.fftshift(np.fft.fft2(x.astype(np.float64)))
...
if __name__ == "__main__":
    ...
    img = rgb2luminance(np.array(Image.open(IMAGE_PATH)))
    print(f"Shape    : {img.shape}")

    figure_analyse(img, TARGET,
                   chemin_sortie=os.path.join(OUT_DIR, "fourier_analysis.png"),
                   direction=DIRECTION)

    figure_4composantes(img, TARGET,
                        chemin_sortie=os.path.join(OUT_DIR, "fourier_4components.png"))
